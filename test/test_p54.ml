...
module HandImplSuite = struct
  ...
  (****************************)
  let value _ =
    let printer = sprintf "0x%x" in

    let msg = "flush" in
    let data = ((2, 1), (5, 1), (3, 1), (4, 1), (7, 1)) in
    let expected = 0x575432 in
  ...
  let compare _ =
    let msg = "draw" in
    let data1 = "8C TS KC 9H 4S" |> P54.HandImpl.of_string in
    let data2 = "8C TS KC 9H 4S" |> P54.HandImpl.of_string in
    let expected = 0 in
    let actual = P54.HandImpl.compare data1 data2 in
    assert_equal ~msg expected actual;

    let msg = "loss" in
    let data1 = "8C 8S KC 9H 4S" |> P54.HandImpl.of_string in
    let data2 = "8C TS 9C 9H 4S" |> P54.HandImpl.of_string in
    let expected = -1 in
    let actual = P54.HandImpl.compare data1 data2 in
    assert_equal ~msg expected actual;

    let msg = "win" in
    let data1 = "8C 8S 8D 8H 4S" |> P54.HandImpl.of_string in
    let data2 = "8C 8S 9C 9H 4S" |> P54.HandImpl.of_string in
    let expected = 1 in
    let actual = P54.HandImpl.compare data1 data2 in
    assert_equal ~msg expected actual;
end
